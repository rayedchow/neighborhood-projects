...
export class ProgressService {
   ...
   */
  public static updateProgress(request: UpdateProgressRequest): ApiResponse<boolean> {
   ...
   */
  public static getRecommendedPractice(
    userId: string,
    courseId: string,
    count: number = 3
   ...
   */
  public static getUserStats(userId: string): ApiResponse<{
   ...
   * @returns ApiResponse indicating success or failure
   */
  public static resetCourseProgress(userId: string, courseId: string): ApiResponse<boolean> {
    try {
      const data = readProgressData();
      const userIndex = data.users.findIndex(u => u.id === userId);
