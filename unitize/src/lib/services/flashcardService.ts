...
// Define the path to the flashcards data file
...
const readFlashcardsData = (): FlashcardsData => {
  try {
    // Create file with default data if it doesn't exist
    if (!fs.existsSync(FLASHCARDS_DATA_PATH)) {
      const defaultData: FlashcardsData = { cards: [], decks: [] };
      fs.writeFileSync(FLASHCARDS_DATA_PATH, JSON.stringify(defaultData, null, 2));
      return defaultData;
    }
    
    const data = fs.readFileSync(FLASHCARDS_DATA_PATH, 'utf-8');
...
const writeFlashcardsData = (data: FlashcardsData): boolean => {
  try {
    fs.writeFileSync(FLASHCARDS_DATA_PATH, JSON.stringify(data, null, 2));
    return true;
  } catch (error) {
...
export class FlashcardService {
  ...
  // Delete a flashcard
  public static deleteFlashcard(userId: string, cardId: string): ApiResponse<boolean> {
    try {
      const data = readFlashcardsData();
      ...
      // Find the deck containing this card to verify ownership
      const deckIndex = data.decks.findIndex(deck => 
        deck.userId === userId && deck.cardIds.includes(cardId)
      );
  ...
  // Delete a deck
  public static deleteDeck(userId: string, deckId: string): ApiResponse<boolean> {
    try {
      const data = readFlashcardsData();
  ...
  public static getUserDecks(userId: string): ApiResponse<FlashcardDeck[]> {
    try {
      const data = readFlashcardsData();
  ...
  public static getDeckWithCards(deckId: string): ApiResponse<{deck: FlashcardDeck, cards: Flashcard[]}> {
    try {
      const data = readFlashcardsData();
      ...
      const cards = data.cards.filter(card => deck.cardIds.includes(card.id));
  ...
  public static getDueCards(userId: string, limit: number = 20): ApiResponse<Flashcard[]> {
    try {
      const data = readFlashcardsData();
      ...
      const userCardIds = data.decks
        .filter(deck => userDeckIds.includes(deck.id))
  ...
  public static getPublicDecks(limit: number = 20, offset: number = 0): ApiResponse<FlashcardDeck[]> {
    try {
      const data = readFlashcardsData();
  ...
  public static cloneDeck(userId: string, deckId: string, newName?: string): ApiResponse<FlashcardDeck> {
    try {
      const data = readFlashcardsData();
      
      // Find the source deck
      const sourceDeck = data.decks.find(deck => deck.id === deckId && deck.isPublic);
      ...
      sourceDeck.cardIds.forEach(cardId => {
        const sourceCard = data.cards.find(card => card.id === cardId);
        if (sourceCard) {
          const newCard: Flashcard = {
            ...sourceCard,
