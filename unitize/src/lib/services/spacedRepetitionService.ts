...
export class SpacedRepetitionService {
   ...
   */
  public static getDueCards(userId: string, limit: number = 20): ReviewCard[] {
    const srData = this.getUserSRData(userId);
    if (!srData) return [];
  ...
  public static processReview(
    ...
    try {
      let srData = this.getUserSRData(userId);
   ...
   */
  private static getUserSRData(userId: string): SpacedRepetitionData | null {
    try {
      // Read from storage
   ...
   * @returns True if successful
   */
  private static saveUserSRData(srData: SpacedRepetitionData): boolean {
    try {
      // Read from storage
      const data = readProgressData();
   ...
   */
  public static getReviewStats(userId: string): {
