...
export default function Dashboard() {
  ...
  useEffect(() => {
    async function fetchDashboardData() {
      ...
      try {
        // Fetch user stats
        const statsPromise = fetch(`/api/progress/stats?userId=${DEMO_USER_ID}`).then(res => res.json());
        
        // Fetch recent courses
        const coursesPromise = fetch(`/api/progress?userId=${DEMO_USER_ID}`).then(res => res.json());
        
        // Fetch spaced repetition stats
        const reviewStatsPromise = fetch(`/api/spaced-repetition?userId=${DEMO_USER_ID}`, {
          method: 'PATCH'
        }).then(res => res.json());
        
        // Fetch goals data
        const goalsPromise = fetch(`/api/goals?userId=${DEMO_USER_ID}`).then(res => res.json());
        
        // Fetch recommendations
        const recommendationsPromise = fetch(`/api/progress/recommend?userId=${DEMO_USER_ID}`).then(res => res.json());
        
        // Wait for all requests to complete
        const [statsRes, coursesRes, reviewStatsRes, goalsRes, recommendationsRes] = await Promise.all([
          statsPromise, coursesPromise, reviewStatsPromise, goalsPromise, recommendationsPromise
        ]);
